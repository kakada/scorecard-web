...
RSpec.describe UnlockRequestPolicy do
  ...
  permissions :update? do
    context "user is proposer and unlock_request is submitted" do
      let(:user) { create(:user, :lngo, local_ngo: lngo, program: program) }
      let(:scorecard) { create(:scorecard, local_ngo_id: lngo.id) }
      let(:unlock_request) { create(:unlock_request, scorecard: scorecard, proposer: user, status: :submitted) }
    ...
    context "user is not proposer" do
      ...
      let(:user) { create(:user, :lngo, local_ngo: lngo, program: program) }
      let(:scorecard) { create(:scorecard, local_ngo_id: lngo.id) }
      let(:unlock_request) { create(:unlock_request, scorecard: scorecard, proposer: proposer, status: :submitted) }
    ...
    context "unlock_request is approved" do
      let(:user) { create(:user, :lngo, local_ngo: lngo, program: program) }
      let(:scorecard) { create(:scorecard, local_ngo_id: lngo.id) }
      let(:unlock_request) { create(:unlock_request, scorecard: scorecard, proposer: user, status: :approved) }
  ...
  permissions :review? do
    context "user is program_admin and unlock_request is submitted" do
      let(:proposer) { create(:user, :lngo, local_ngo: lngo, program: program) }
      let(:user) { User.new(role: :program_admin, program_id: program.id) }
    ...
    context "user is staff and unlock_request is submitted" do
      let(:proposer) { create(:user, :lngo, local_ngo: lngo, program: program) }
      let(:user) { User.new(role: :staff, program_id: program.id) }
      let(:scorecard) { create(:scorecard, local_ngo_id: lngo.id, program: program) }
      let(:unlock_request) { create(:unlock_request, scorecard: scorecard, proposer: proposer, status: :submitted) }
    ...
    context "unlock_request is approved" do
      ...
      let(:user) { User.new(role: :program_admin, program_id: program.id) }
      let(:scorecard) { create(:scorecard, local_ngo_id: lngo.id, program: program) }
      let(:unlock_request) { create(:unlock_request, scorecard: scorecard, proposer: proposer, status: :approved) }
