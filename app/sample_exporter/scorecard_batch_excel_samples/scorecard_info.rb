...
module ScorecardBatchExcelSamples
  class ScorecardInfo < Base
    private
      def build_headers
        row = ["year", "facility_code", "scorecard_type_en", "running_mode_en", "location_code"]
      ...
      def build_rows
        ...
        row = [ Date.today.year, facility.try(:code) || "HC", Scorecard.scorecard_types.keys.sample, Scorecard.running_modes.keys.sample, "010201" ]
        ...
        sheet.add_row row, types: [:integer, :string, :string, :string, :string, :string, :string]
      end
